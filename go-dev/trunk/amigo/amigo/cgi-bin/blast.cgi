...
$core->kvetch("Starting blast.cgi...");
...
if ($cgi_error){
  ...
  if ($cgi_error eq '413 Request entity too large') {
    $cgi_error = 'The file you uploaded was too large. ' .
      'The maximum size for file uploads is '.$max_size.' Kb.';
  }
...
if ($session->ses_type eq 'blast_query'){
  ...
  foreach my $x ('blast_filter', 'threshold', 'maxhits'){
    ...
    $vars->{extra_filter}{$x} =
      $session->set_option_with_value($x, $vars->{$x});
  }
  #$core->kvetch("starting process_page_template...");
  process_page_template($vars, $session);
  #$core->kvetch("processed page template.");
  exit;
  ...
  if ($q->upload('seq_file_upload')){
    $core->kvetch("Uploaded seq file found");
  ...
  my $result_h = set_off_blast($session->apph, $vars->{error},
			       $blast_settings, $input_h, $vars);
  ...
  my $blasted = check_blast($session->get_blast_results_dir,
			    scalar @{$inputs->{seqs}});

  if ($blasted == 1) {
    $core->kvetch("BLAST finished... getting the results.");
  ...
  if (!$cache->{input}){
    $core->kvetch("lost the cache settings!");
    $vars->{error} =
  ...
  $core->kvetch("cache loaded: ". Dumper($cache));
...
sub fetch_results {
  ...
  my $result_h = get_blast_results($session->apph, $vars->{error},
				   $session->get_blast_results_dir, $page,
				   $session->id, $cache_hash->{use_filters});
  $vars->{error} = $result_h->{error};

  if (!$result_h->{results}){
    #$core->kvetch("blast.cgi: No results for blast query found");
    $session->ses_type('amigo_message');
  ...
  $session->save_cached_results($result_h->{to_cache}, 'blast_scores')
...
sub check_blast {
  ...
  $core->kvetch("looking for $result_file...");

  if (-e $result_file && -f $result_file) {
    $core->kvetch("Found the file!");
    $retval = 1;
  } else {
    $core->kvetch("Could not find the file!");
  }
