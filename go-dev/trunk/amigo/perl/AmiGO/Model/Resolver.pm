...
This should be the only stop for turning things into AmiGO internal
canonical form.
...
package AmiGO::Model::Resolver;

use base 'AmiGO';
use utf8;
use strict;
use AmiGO::Model::Schema;
use AmiGO::Model::Query;
...
sub new {
  ...
  $self->{GP_Q} = AmiGO::Model::Query->new({type=>'gene_product_lazy'});
  $self->{GP_SYN_Q} = AmiGO::Model::Query->new({type=>'gene_product_synonym'});
  $self->{DBXREF_Q} = AmiGO::Model::Query->new({type=>'dbxref'});
  $self->{TERM_Q} = AmiGO::Model::Query->new({type=>'term_lazy'});
  $self->{TERM_SYN_Q} = AmiGO::Model::Query->new({type=>'term_synonym'});
