...
$GO_ROOT/go-db-perl/scripts/go_db_install.pl -F -i -n -j -e $GO_DBNAME -v -d $GO_DBHOST; cd $GO_CVS/gene-associations/submission/paint; cvs update; find . -name "*.gaf" | xargs -I {} perl -I $GOBO_ROOT -I $GO_ROOT/go-db-perl -I $GO_ROOT/go-perl $GO_ROOT/go-db-perl/scripts/load-go-into-db.pl -datatype go_assoc -h $GO_DBHOST -d $GO_DBNAME {}
