;; obo-mode.el --- a major mode for editing obo files
;;
;;
;; $Id: obo-mode.el,v 1.3 2007/05/03 23:03:40 cmungall Exp $
;;
;;
;; Adapted by Chris Mungall from DNA sequence mode:
...
(defun obo-setup (namespace db-prefix last-id)
  "initalises vars"
  (interactive "snamespace: \nsdb-prefix: \nnlast-id: ")
  (setq obo-namespace namespace)
  (setq obo-last-id last-id)
...
(defun obo-add-id (name-init)
  "adds an ID based on lookup by name"
  (interactive "sEnter first 3 or or more letters of term: ")
  (setq name
        (completing-read
         "Term name or syn: "
         (mapcar (lambda(id-name) (list (cdr id-name) 1)) obo-lookup-table) 
         nil t name-init))
  (insert (car (rassoc name obo-lookup-table))))

(defun obo-add-id-name (name-init)
  "adds an ID based on lookup by name"
  (interactive "sEnter first 3 or or more letters of term: ")
  (setq name
        (completing-read
         ...
         (mapcar (lambda(id-name) (list (cdr id-name) 1)) obo-lookup-table) 
         nil t name-init))
  (insert (format "%s {%s}" (car (rassoc name obo-lookup-table)) name)))
