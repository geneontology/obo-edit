# $Id: Admin.pm,v 1.49 2010/03/25 16:36:09 benhitz Exp $
#
...
# gets the goa_uniprot GA file which is huge and been removed from CVS.  Needed for gofull.
# must be run after this directory is purged prior to CVS update.
sub get_goa_uniprot_ga {
    my $self = shift;
    my $data_root = $self->data_root;
    my $target = "$data_root/gene-associations";
    
    $self->runcmd("wget -r -np -nv -nd -P $target ftp://ftp.geneontology.org/go/gene-associations/gene_association.goa_uniprot.gz");
