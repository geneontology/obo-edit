...
there is only a target for GO.
  ...
  Usage:
     indexer.pl [-h] [-v] [-x <target>]

  Options:
     -h               Print this message.
     -v               Enable more verbose messages.
     -x <target>      Name of target. Values are 'toy' and 'live'

  Example Usage:
     perl indexer.pl -x toy
...
EOC
  ...
  if ( $opt_x && (
    ...
    die "Target (-x) is a required argument: $!";
  }else{
    die "Your target (-x) is illegitimate: $!";
  }
  ...
  ##
  if( $local{TARGET} eq 'toy' ){

    #$results_to_use = get_from_local_db();
    #$to_url = \&amigo_url_toy;
    $results_to_use = get_from_ebi_db();
    $to_url = \&amigo_url_toy;

  }elsif( $local{TARGET} eq 'live' ){

    $results_to_use = get_from_ebi_db();
    $to_url = \&amigo_url_live;
...
sub get_from_local_db{
  ...
  ## Connect to DB.
  my $db_handle =
    DBI->connect("DBI:mysql:go_latest:spitz", "", "") ||
...
sub get_from_ebi_db{
  ...
  ## Connect to DB.
  my $db_handle =
    DBI->connect("DBI:mysql:go_latest:mysql.ebi.ac.uk:4085",
		 "go_select", "amigo") ||
      die_template("Couldn't connect to that database on that host.");
...
sub amigo_url_live {
  my $var = shift || '_INDEXER_ERROR_';
  return 'http://amigo.geneontology.org/cgi-bin/amigo/go.cgi?view=details&search_constraint=terms&depth=0&query=GO:0032502=' . $var . '';
}
