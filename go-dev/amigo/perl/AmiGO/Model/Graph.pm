...
sub new {
  ...
  $self->{SCHEMA} = AmiGO::Model::Schema->connect($self->db_connector());
  $self->{GRAPH_Q} =
    AmiGO::Model::Query->new({type=>'term2term_over_graph_path'});
...
sub get_children {
  ...
  foreach my $t2t (@$all){
    if( ! $t2t->subject->is_obsolete ){
      ...
      $self->kvetch("AmiGO::Model::Graph::get_children: kid: " .
		    $t2t->subject->acc);
    }
  }
  return $ret;
