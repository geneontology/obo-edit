...
public class AdminServlet extends HttpServlet {
		...
		for (String opName : task.getCompletedOperations()) {
			...
			if(st.length>1){
				if(!ontology.equals(st[1])){
					ontology = st[1];
					writer.write("<tr><td colspan='2'><h4>Loading " + ontology + "</h4></td></tr>");
				}
	...
	/*private OWLGraphWrapper buildOWLGraphWrapper(String ontologylocation)
			throws IOException {
		try {
			return new DbOperations().buildOWLGraphWrapper(ontologylocation);
		} catch (Exception ex) {
			throw new IOException(
					"An error ocurred while building OWLGraphWrapper object reference",
					ex);
		}
	}*/
		...
		if (command != null && task == null) {
			if ("update".equals(command)) {
				...
				if (ontologylocation == null) {
					...
					if (graph == null) {
						writer.write("<h1>OWLGraphWrapper cannot be built from the ontologylocation at '"
								+ ontologylocation + "'");
					} else {
						graphs.put(graph.getOntologyId(), graph);*/
						addReload = true;
