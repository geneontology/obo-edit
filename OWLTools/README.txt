...
The command 'ant make-obo-release' builds an ontology release whose information is configured in the
conf/obo-release.properties file. See the obo-release.properties file for further documentation about the 
configuration options. All files and directories generated by this tool are maintained under the oborelease/releases directory.
...
by the date of the release. With large ontologies the ant tool could throw OutofMemoryError exception. With setting the 
export ANT_OPTS="-Xms1000m -Xmx3000m" environment variable fix the problem.
